...
const TESTIMONIALS = [
  ...
  { name: 'Shreya Reddy', college: 'AIIMS Delhi', review: 'Got Campbell Biology at 60% off. The seller was from my own college! CampusBazaar understands what students need.', rating: 5 },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        >
          ...
          <p className="text-muted-foreground max-w-xl mx-auto">
            Hear from students who saved thousands using CampusBazaar
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {TESTIMONIALS.map((t, i) => (
            <motion.div
              key={t.name}
              className="p-6 rounded-2xl bg-card border border-border hover:shadow-md transition-all duration-300 relative"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.4, delay: i * 0.08 }}
            >
              <Quote className="w-8 h-8 text-brand/15 absolute top-4 right-4" />
              ...
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-brand to-blue-700 flex items-center justify-center text-white text-sm font-bold">
                  {t.name.charAt(0)}
                </div>
                <div>
