...
import { Users, BookOpen, AlertTriangle, Star, TrendingUp, Eye, Trash2, Ban, BadgeCheck, ArrowLeft, RefreshCw } from 'lucide-react'
...
const CHART_COLORS = ['#2563eb', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4', '#f97316', '#ec4899', '#6366f1']
    ...
    return (
      <div className="pt-20 pb-10 min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => setCurrentPage('home')} className="btn-gradient text-white border-0">Go Home</Button>
        </div>
      </div>
    )
  }
        ...
        {loading || !stats ? (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {Array.from({ length: 4 }).map((_, i) => (
              <Card key={i} className="p-6"><div className="h-16 bg-muted animate-pulse rounded" /></Card>
            ))}
          </div>
        ) : (
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              {[
                ...
                { icon: BookOpen, label: 'Total Listings', value: stats.totalListings, color: 'from-emerald-500 to-teal-500' },
                { icon: AlertTriangle, label: 'Active Reports', value: stats.unresolvedReports, color: 'from-red-500 to-rose-500' },
                { icon: Eye, label: 'Total Views', value: stats.totalViews, color: 'from-purple-500 to-violet-500' },
              ].map((stat, i) => (
                <motion.div
                  key={stat.label}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              <Card className="p-6">
                <h3 className="text-sm font-semibold text-foreground mb-4">Listings by City</h3>
              ...
              <Card className="p-6">
                <h3 className="text-sm font-semibold text-foreground mb-4">Listings by Category</h3>
                <ResponsiveContainer width="100%" height={250}>
                  <PieChart>
            ...
            <Tabs defaultValue="reports">
              <TabsList>
                <TabsTrigger value="reports" className="gap-2"><AlertTriangle className="w-4 h-4" /> Reports ({stats.unresolvedReports})</TabsTrigger>
                <TabsTrigger value="users" className="gap-2"><Users className="w-4 h-4" /> Users ({stats.totalUsers})</TabsTrigger>
                <TabsTrigger value="recent" className="gap-2"><TrendingUp className="w-4 h-4" /> Recent</TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="reports" className="mt-6">
                {reports.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {reports.map(report => (
                      <Card key={report.id} className={`p-4 ${report.isResolved ? 'opacity-50' : ''}`}>
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {report.isResolved ? (
                                ...
                                <Badge className="bg-red-500 text-white border-0 text-[10px]">Open</Badge>
                              )}
                            </div>
                          ...
                          {!report.isResolved && (
                            <Button size="sm" variant="outline" onClick={() => handleResolveReport(report.id)} className="shrink-0">
                              Mark Resolved
                            </Button>
                          )}
                        </div>
                ...
                <div className="space-y-3">
                  {users.map(user => (
                    <Card key={user.id} className="p-4">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-brand to-blue-700 flex items-center justify-center text-white font-bold text-sm shrink-0">
                            {user.name.charAt(0)}
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              {user.isAdmin && <Badge className="bg-brand text-white border-0 text-[10px]">Admin</Badge>}
                              {user.isBanned && <Badge className="bg-red-500 text-white border-0 text-[10px]">Banned</Badge>}
                            </div>
                    ...
                    <Card key={listing.id} className="p-4">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        ...
                        <div className="flex gap-2 shrink-0">
                          <Button size="sm" variant="outline" onClick={() => handleVerifyListing(listing.id, false)} className="text-xs gap-1">
                            <BadgeCheck className="w-3 h-3" /> Verify
                          </Button>
                          <Button size="sm" variant="ghost" onClick={() => handleDeleteListing(listing.id)} className="text-xs text-destructive gap-1">
